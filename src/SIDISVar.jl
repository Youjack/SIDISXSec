...
get_ξmin_xB(y, xB, ζ=1) = (1 - y)/( ζ - xB * y )
get_ζmin_zh(y, zh, ξ=1) = (1 - y)/( ξ - zh * y )
get_ξmin_zh(y, zh, ζ=1) = zh * y + (1 - y)/ζ
get_ζmin_xB(y, xB, ξ=1) = xB * y + (1 - y)/ξ
get_ξmins(y, xB, zh, ζ=1) = ( get_ξmin_xB(y, xB, ζ), get_ξmin_zh(y, zh, ζ) )
get_ζmins(y, xB, zh, ξ=1) = ( get_ζmin_xB(y, xB, ξ), get_ζmin_zh(y, zh, ξ) )
...
function cumdf(f::Function, If::Function, D::Function, ID::Function,
    ...
    ξmin(ζ) = isnan(zh) ? get_ξmin_xB(y, xB, ζ) : max(get_ξmins(y, xB, zh, ζ)...)
    ζmin    = isnan(zh) ? get_ζmin_xB(y, xB   ) : max(get_ζmins(y, xB, zh   )...)
